...
class Stats(commands.Cog):
    ...
    async def stats(self, ctx):
        ...
        text_channel_count = len(
            [c for c in self.bot.get_all_channels() if isinstance(c, discord.TextChannel)]
        )
        voice_channel_count = len(
            [c for c in self.bot.get_all_channels() if isinstance(c, discord.VoiceChannel)]
        )
        category_channel_count = len(
            [c for c in self.bot.get_all_channels() if isinstance(c, discord.CategoryChannel)]
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"Category: **{category_channel_count}**"
            ),
            inline=False,
        )
        ...
        embed.add_field(
            name="🤖Commands",
            value=f"Total: **{commands_count}**   |   Slash: **{slash_commands}**",
            inline=False,
        )
        embed.add_field(
            name="🐍 Libraries Used",
            value=f"Discord Library: **[discord.py](https://discordpy.readthedocs.io/en/stable/)**\n"
                  f"Total Libraries: **{total_libraries}**",
            inline=False,
        )
        embed.add_field(
            name=" Codebase Stats",
        ...
        embed.add_field(
            name="🎧 Music Stats",
            value=(
                f"Currently Connected: **[{channels_connected}](https://discord.gg/odx)**\n"
        ...
        embed.set_footer(
            text="Powered by Kyra✨ Development™",
            ...
            if interaction.user == ctx.author:
                ...
                system_embed.add_field(
                    ...
                    value=(
                        ...
                        f"• Memory Left: **{memory_info.available / (1024 ** 2):,.2f} MB**\n"
                        f"• Heap Total: **{memory_info.used / (1024 ** 2):,.2f} MB**"
                    ),
                    inline=False,
                )
                system_embed.add_field(
                    name=" CPU Info",
                    value=(
                        f"• CPU: **{psutil.cpu_freq().max}' GHz**\n"
                        f"• CPU Usage: **{psutil.cpu_percent()}%**\n"
                        f"• CPU Cores: **{psutil.cpu_count(logical=False)}**\n"
                        f"• CPU Speed: **{cpu_info.current:.2f} MHz**"
                    ),
                    inline=False,
                )
        ...
        async def ping_button_callback(interaction):
            if interaction.user == ctx.author:
                ...
                ping_embed.add_field(
                    name="🏓 Bot Latency",
                    value=f"{round(sh.latency * 800)} ms",
                    inline=False,
                )
                ping_embed.add_field(
                    name="📦 Database Latency", value=f"{db_latency} ms", inline=False
                )
                ping_embed.add_field(
                    name="📦 Websocket Latency", value=f"{wsping} ms", inline=False
                )
        ...
        async def team_button_callback(interaction):
            if interaction.user == ctx.author:
                ...
                team_embed.add_field(
                    name="**👑 Bot Owner(s)**",
                    value=(
                        ">>> **[Natrix](https://discord.com/users/1341478551764860958)**, "
                ...
                team_embed.add_field(
                    name="**🤖 Bot Developer(s)**",
                    value="> **[Natrix!](https://discord.com/users/1341478551764860958)** (Lead Developer)",
                    inline=False,
                )
                team_embed.add_field(
                    name="**🪫 Web Developer(s)**",
                    value="> **[Natrix](https://discord.com/users/1341478551764860958)** (Lead Web Developer)",
                    inline=False,
                )
                team_embed.add_field(
                    name="**🥲 Tester(s)**",
                ...
                team_embed.add_field(
                    name="**😄 Partner(s)**",
                    value="> **[Endercloud](https://endercloud.in/)**",
                    inline=False,
                )
        ...
        server_count_button = Button(
            label=f"Servers: {guild_count}    |    Users: {blahh}",
